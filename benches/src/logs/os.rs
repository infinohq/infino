...
use opensearch::{
  ...
  params::Refresh,
  BulkParts, Error, IndexParts, OpenSearch,
};
...
impl OpenSearchEngine {
  ...
  pub async fn index_lines(&self, input_data_path: &str, max_docs: i32) -> u128 {
          ...
          if num_docs_in_this_batch == num_docs_per_batch {
            ...
            if bytes_sent_since_flush > FLUSH_LIMIT_BYTES {
              ...
              let flush = self
                ...
                .unwrap();
              println!(
                "Flushed index {} after writing {} bytes",
                INDEX_NAME, bytes_sent_since_flush
              );
    ...
    println!(
      "OpenSearch time required for insertion: {} microseconds",
      elapsed
    );
  ...
  pub async fn get_index_size(&self) -> u64 {
    ...
    println!(
      "OpenSearch cat response to figure out index size: {}",
      cat_response
    );
    ...
    println!("Could not programmatically figure out opensearch index size. Figure it out from the cat response printed above");
    0
  }
  ...
  pub async fn search(&self, query: &str) -> u128 {
    ...
    println!(
      "OpenSearch time required for searching query {} is : {} microseconds",
      query, elapsed
    );
